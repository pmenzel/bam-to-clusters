...
int processDepths(mplp_data ** data, int n_files, char * reg, int min_phred, FILE * of, double norm_factor, int smart_overlap, int fraction_counts, double min_height, int min_dist, int min_length) {
	bam_mplp_t mplp;
	int tid;
	int pos;
	int f,i;
	...
	char * curr_targetname = NULL;

	int curr_plus_start = 0;
	int curr_plus_end = 0;
	double curr_plus_max_height = 0;
	int curr_minus_start = 0;
	int curr_minus_end = 0;
	double curr_minus_max_height = 0;
	...
	while((ret = bam_mplp_auto(mplp, &tid, &pos, n_plp, plp)) > 0) {
		...
		if(tid != ctid) {
			if(curr_plus_max_height >= min_height && curr_plus_end - curr_plus_start > min_length) {
				fprintf(of, "%s\t%i\t%i\t+\t%.3f\n", curr_targetname, curr_plus_start + 1, curr_plus_end + 1, curr_plus_max_height);
			}
			if(curr_minus_max_height >= min_height && curr_minus_end - curr_minus_start > min_length) {
				fprintf(of, "%s\t%i\t%i\t-\t%.3f\n", curr_targetname, curr_minus_start + 1, curr_minus_end + 1, curr_minus_max_height);
			}
			curr_plus_start = 0;
			curr_plus_end = 0;
			curr_plus_max_height = 0.0;
			curr_minus_start = 0;
			curr_minus_end = 0;
			curr_minus_max_height = 0.0;
			curr_targetname = h->target_name[tid];
			ctid = tid;
		}
		for(f=0;f<n_files;f++) {
		...
		if(plus_depth > 0.0) {
			if(pos + 1 - min_dist > curr_plus_end) { // this is the start of a new cluster
				//check if previous cluster fulfills requirements
				if(curr_plus_max_height >= min_height && curr_plus_end - curr_plus_start > min_length) {
					fprintf(of, "%s\t%i\t%i\t+\t%.3f\n", curr_targetname, curr_plus_start + 1, curr_plus_end + 1, curr_plus_max_height);
				}
				curr_plus_start = pos;
				curr_plus_end = pos;
				curr_plus_max_height = plus_depth;
			}
			else { // continue current cluster
				curr_plus_end = pos;
				if(plus_depth > curr_plus_max_height) {
					curr_plus_max_height = plus_depth;
				}
			}
		}
		...
		if(minus_depth > 0.0) {
			if(pos + 1 - min_dist > curr_minus_end) { // this is the start of a new cluster
				//check if previous cluster fulfills requirements
				if(curr_minus_max_height >= min_height && curr_minus_end - curr_minus_start > min_length) {
					fprintf(of, "%s\t%i\t%i\t-\t%.3f\n", curr_targetname, curr_minus_start + 1, curr_minus_end + 1, curr_minus_max_height);
				}
				curr_minus_start = pos;
				curr_minus_end = pos;
				curr_minus_max_height = minus_depth;
			}
			else { // continue current cluster
				curr_minus_end = pos;
				if(minus_depth > curr_minus_max_height) {
					curr_minus_max_height = minus_depth;
				}
			}
...
int main(int argc, char *argv[]) {
	...
	int c;
	while((c = getopt(argc, argv, "s:m:p:r:o:l:hdvfnxy:")) >= 0) {
		switch(c) {
			case 'o' :
				outputfilename = malloc(strlen(optarg)+1);
				if(outputfilename==NULL) {
					fprintf(stderr,"Error while copying output file name :(\n");
					return 1;
				}
			...
			case 'l' :
				...
				if(min_dist < 1) {
					fprintf(stderr, "Error: -l must be > 0\n");
					usage(argv[0]);
					return 1;
				}
				break;
	...
	if(outputfilename==NULL) {
		fprintf(stderr,"Error: Need to specify the name of the output file with option -o.\n");
		usage(argv[0]);
		return 1;
	}
	...
	if(!(norm_rpm || fraction_counts) && min_height < 1.0) {
		fprintf(stderr,"Warning: Setting minimum height (option -y) to 1.0\n");
		min_height = 1.0;
	}
	...
	ret = processDepths(data, n_files, reg, min_phred, of, norm_factor, smart_overlap, fraction_counts, min_height, min_dist, min_length);
...
void usage(char *prog) {
	fprintf(stderr, "Usage: %s -o outputfilename [-s 1|2] [-r STRING] [-p INT] [-m INT] <in1.bam> [<in2.bam> ...]\n",prog);
	fprintf(stderr, "\n");
	fprintf(stderr, "This program reads sorted BAM files containing mapped reads from stranded RNASeq libraries\n");
	fprintf(stderr, "and find clusters of mapped reads.\n");
	fprintf(stderr, "\n");
	fprintf(stderr,	"Options:\n");
	fprintf(stderr,	"-o STRING  name of output file\n");
	fprintf(stderr,	"\n");
	...
	fprintf(stderr,	"\n");
	fprintf(stderr,	"-l INT     Minimum distance between clusters. Default: 10\n");
	fprintf(stderr,	"\n");
	fprintf(stderr,	"-y FLOAT   Minimum height of cluster (corresponds to read depth or RPM). Default: 1.0\n");
	fprintf(stderr,	"\n");
